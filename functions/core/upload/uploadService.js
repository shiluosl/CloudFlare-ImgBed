...
export class UploadService {
  ...
  async upload(input) {
    ...
    if (policy.stop_when_quota_risk && level !== 'NORMAL') {
      const error = new Error('This storage policy pauses uploads when a Zero-Cost quota risk is detected');
      error.code = 'QUOTA_RISK_POLICY';
      error.status = 503;
      error.level = level;
      throw error;
    }
...
function estimateMetadataBytes(file, replicas) {
  const fileMetadata = 2048 + Math.min(String(file.name || '').length, 240) * 2;
  return fileMetadata + replicas.length * 2048;
}
