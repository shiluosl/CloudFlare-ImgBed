...
export class ZeroCostGuard {
  ...
  async assertQueue({ essential = false } = {}) { const { level } = await this.status(); if (['EMERGENCY', 'READ_ONLY'].includes(level) || (level === 'WRITE_LIMITED' && !essential)) throw forbidden(level, 'Queue dispatch is paused by the Zero-Cost Guard'); return level; }
