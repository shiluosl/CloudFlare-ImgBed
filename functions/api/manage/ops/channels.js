...
import { effectiveChannelCapabilities } from '../../../core/storage/capabilities.js';
...
function operationError(error) { return Response.json({ error: error.code === 'ZERO_COST_GUARD' ? 'Operation is paused by the Zero-Cost Guard' : error.message || 'Invalid channel configuration', code: error.code || 'INVALID_CONFIGURATION', protectionLevel: error.level || null }, { status: error.code === 'ZERO_COST_GUARD' ? 503 : 400 }); }
