...
export class TelegramAdapter extends StorageAdapter {
  ...
  token() { return this.env?.[this.channel.secretRefs?.tokenRef] || this.env?.[this.channel.secret_refs?.tokenRef] || this.config().token; }
  endpoint(path) { const base = this.config().proxyUrl ? `https://${this.config().proxyUrl}` : 'https://api.telegram.org'; return `${base}/bot${this.token()}/${path}`; }
  fileEndpoint(path) { const base = this.config().proxyUrl ? `https://${this.config().proxyUrl}` : 'https://api.telegram.org'; return `${base}/file/bot${this.token()}/${path}`; }
  ...
  async api(path, init, operation) { this.ensureConfig(); const response = await withTimeout(this.fetch, this.endpoint(path), init, Number(this.config().timeoutMs) || 10000, { provider: this.provider(), channelId: this.channel.id }); if (!response.ok) throw errorForResponse(this.provider(), this.channel.id, response, operation); const data = await response.json(); if (!data.ok) throw telegramError(this.channel.id, operation, data, response.status); return data.result; }
  async put(input) {
  ...
  async get(input) { const info = await this.fileInfo(input.remoteId); const response = await withTimeout(this.fetch, this.fileEndpoint(info.file_path), { method: 'GET', headers: input.range ? { Range: input.range } : {} }, Number(this.config().timeoutMs) || 10000, { provider: this.provider(), channelId: this.channel.id }); if (!response.ok) throw errorForResponse(this.provider(), this.channel.id, response, 'GET'); return response; }
