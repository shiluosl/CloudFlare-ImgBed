...
export class WebDavAdapter extends StorageAdapter {
  ...
  headers(extra = {}) {
    ...
    const username = this.env?.[this.channel.secret_refs?.usernameRef] || this.env?.[this.channel.secretRefs?.usernameRef] || config.username;
    const password = this.env?.[this.channel.secret_refs?.passwordRef] || this.env?.[this.channel.secretRefs?.passwordRef] || config.password;
  ...
  async ensureDirectory(key) { if (this.config().createDirectory === false) return; const parts = String(key).split('/').filter(Boolean); parts.pop(); let path = ''; for (const part of parts) { path = path ? `${path}/${part}` : part; const response = await withTimeout(this.fetch, this.objectUrl(path), { method: 'MKCOL', headers: this.headers(), redirect: 'manual' }, Number(this.config().timeoutMs) || 10000, { provider: this.provider(), channelId: this.channel.id }); if (![200, 201, 204, 405].includes(response.status)) throw errorForResponse(this.provider(), this.channel.id, response, 'MKCOL'); } }
