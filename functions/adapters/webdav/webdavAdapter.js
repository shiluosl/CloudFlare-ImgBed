...
import { buildWebDAVUrl, fetchWebDAVRead, normalizeBaseUrl } from '../../utils/storage/webdavAPI.js';
...
export class WebDavAdapter extends StorageAdapter {
  ...
  async readRequest(key, init, operation) {
    const detail = { provider: this.provider(), channelId: this.channel.id };
    const timeoutMs = Number(this.config().timeoutMs) || 10000;
    let response;
    try {
      response = await fetchWebDAVRead(this.objectUrl(key), {
        ...init,
        headers: this.headers(init.headers),
        redirect: 'manual',
      }, (url, request) => withTimeout(this.fetch, url, request, timeoutMs, detail));
    } catch (error) {
      if (error instanceof StorageError) throw error;
      throw new StorageError({ ...detail, code: STORAGE_ERROR_CODES.INVALID_CONFIGURATION, message: 'WebDAV read redirect rejected', cause: error });
    }
  ...
  async get(input) { return this.readRequest(input.objectKey, { method: 'GET', headers: input.range ? { Range: input.range } : {} }, 'GET'); }
  async head(input) { const response = await this.readRequest(input.objectKey, { method: 'HEAD' }, 'HEAD'); return { exists: true, size: Number(response.headers.get('Content-Length')) || null, etag: response.headers.get('ETag'), checksum: response.headers.get('Digest') || null }; }
