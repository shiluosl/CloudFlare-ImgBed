...
Post-release compatibility hotfix completed on 2026-07-24. Legacy WebDAV reads now support the bounded HTTPS-only signed-download redirects used by 123pan: authorization, cookies, proxy authorization, and host headers are removed before following the redirect, private-network targets are rejected, and provider response details are not returned to the caller. A subsequent production-read incident showed that the signed 123pan download origin intermittently returns HTTP 502 even though the same logical file is otherwise readable. The legacy reader therefore retries exactly once for GET/HEAD network failures and 5xx responses, restarting from the authenticated WebDAV URL to receive a fresh signed download URL. It never retries 4xx responses, mutations, invalid redirects, or unsafe endpoints. Legacy file misses now use `private, no-store, max-age=0`, preventing a transient pre-upload or upstream 404 from remaining in a browser cache for one day after the file becomes available. Before this retry fix, six real reads of `hbxALQ38.png` produced three HTTP 502s and three HTTP 200s, confirming an external-source availability issue rather than a Telegram referer or legacy anti-hotlink rule. The prior production Worker version was `20b1ddd5-6287-4daf-8c8c-f9c35a3726de`; the legacy files `j5p6L6vM.png` and `hbxALQ38.png` were verified through the custom domain with HTTP 200, `image/png`, and 34,590/52,186 bytes respectively. The Worker default-domain check timed out from the local network, while the custom domain reached the deployed Worker successfully.
