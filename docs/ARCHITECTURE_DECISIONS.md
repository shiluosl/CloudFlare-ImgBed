...
## ADR-036: V3 reuses the upstream uploader through a browser compatibility bridge

The V3 portal uses the existing Vue uploader for its login, drag-and-drop, queue, progress, history, retry, preview, and copy-link experience. The bridge activates only on `?v3=1`, preserves the upstream session flow, and rewrites its same-origin single-file `POST /upload` XHR calls to `/api/upload/v3` with an idempotency key and a compatibility-only header. The header only selects the response envelope expected by the mature Vue client; it grants no permission and does not alter V3 authentication, policy selection, storage routing, Zero Cost Guard, auditing, or replica status. Chunked uploads, remote URL imports, Hugging Face direct uploads, and cross-origin provider writes are blocked because the V3 safe flow does not support them. An extensionless `/v3-upload` route redirects to `/?v3=1`; the bridge also corrects that path before Vue starts when a local Assets SPA fallback is served ahead of the Worker function route. This avoids a second UI while keeping the V3 service authoritative for all data-plane decisions.
