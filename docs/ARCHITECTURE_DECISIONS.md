...
The `worker_requests` counter is an application estimate used by the Zero Cost Guard, not a billing substitute. The deployed Worker samples V3 public-read, operations API, Queue, and scheduled invocations at `WORKER_REQUEST_SAMPLE_RATE` (default `100`) and records the corresponding estimated increment asynchronously with `waitUntil`. This prevents high-volume `/file/{fileId}` reads from causing a D1 write per request while still making the protection panel and `WORKER_REQUEST_SOFT_LIMIT` meaningful. A rate-paused synchronous channel is also rejected before logical-file creation, so a known Telegram/WebDAV `Retry-After` interval cannot create an immediately stranded file.
